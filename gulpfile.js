...
gulp.task('default', function() {
  ...
  gulp.watch(['*.html', '*.md', '_layouts/*.html', '_includes/*.html', '_posts/*', '_config.yml', '!_site/*'], ['build', browserSync.reload]);
...
gulp.task('svg', function() {
  return gulp.src(srcAssets.svg + '*')
    ...
    .pipe($.imagemin())
    .pipe($.svgstore({ fileName: 'sprite.svg' }))
...
gulp.task('cleanStyles', function (cb) {
  del('assets/dist/stylesheets/*.css', cb);
});

gulp.task('cleanScripts', function (cb) {
  del('assets/dist/scripts/main*', cb);
});

gulp.task('cleanVendorScripts', function (cb) {
  del('assets/dist/scripts/vendor*', cb);
});
...
gulp.task('injectVendorScripts', ['vendorScripts'], function () {
  var target = gulp.src('_layouts/default.html');
  var sources = gulp.src('assets/dist/scripts/vendor*', {read: false});
